...
def get_parser():
    ...
    parser.add_argument(
        "--autogenerated-flag-label",
        type=non_empty_string,
        default="md2cf-generated",
        help="Label indicating the page is auto-generated. Must be a non-empty string if provided."
    )

    parser.add_argument(
        "--check-autogenerated-label",
        action="store_true",
        help="Check for a 'marker' label indicating if the page is autogenerated.",
    )
...
def validate_autogenerated_pages(
        confluence: api.MinimalConfluence, 
        space, 
        pages_to_upload,
        generated_label):
    """
    This function retrieves all pages from a space and then checks if any of the 
    pages to be uploaded share the title with one of the existing pages in the space. 
    If such pages are found, it checks if they have the generated_label. If they do not, 
    it returns an error with the list of the pages.

    Args:
        confluence (MinimalConfluence): The MinimalConfluence instance.
        space (str): The space key where to retrieve pages from.
        pages_to_upload (list): The list of pages to upload.
        generated_label (str): The label indicating the page is auto-generated.

    Returns:
        error (str), pages_without_label (list): An error message and a list of pages without the generated_label.
    """
    existing_pages = confluence.get_all_pages_from_space(
        space, 
        expand="metadata.labels",
        limit=500
    )
    existing_pages_titles = {page.title.lower(): page for page in existing_pages}

    pages_without_label = []

    for page in pages_to_upload:
        if page.title.lower() in existing_pages_titles:
            existing_page = existing_pages_titles[page.title.lower()]
            if not any(label.name == generated_label for label in existing_page.metadata.labels.results):
                pages_without_label.append(page.title)

    if pages_without_label:
        error = f"Some pages are going to be updated but they are not marked with '{generated_label}' label."
        return error, pages_without_label
...
def main():
    ...
    if args.check_autogenerated_label:  # If validation is activated
        validation_error, pages_without_label = validate_autogenerated_pages(
            confluence=confluence,
            space=args.space,
            pages_to_upload=pages_to_upload,
            generated_label=args.autogenerated_flag_label
        )

        # If the validation failed, ask the user what to do
        if validation_error:  
            print("Validation failed for the following pages: ")

            # Creating table with rich
            table = rich.table.Table(title="Pages without label")

            table.add_column("Title", justify="left", style="cyan")

            for page in pages_without_label:
                table.add_row(page)

            console.print(table)

            user_input = input("Do you want to override these existing pages without the label? (yes/no): ")
        ...
        for page in pages_to_upload:
            ...
            try:
                ...
                if not args.dry_run:
                    ...
                    extra_labels = args.extra_labels if args.extra_labels else []
                    if args.check_autogenerated_label:
                        autogenerated_label = args.autogenerated_flag_label
                        if autogenerated_label not in extra_labels:
                            extra_labels.append(autogenerated_label)
